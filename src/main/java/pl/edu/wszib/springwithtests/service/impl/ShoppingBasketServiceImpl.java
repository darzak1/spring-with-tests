...
import pl.edu.wszib.springwithtests.NotFoundExpeption;
...
public class ShoppingBasketServiceImpl implements ShoppingBasketService {
    ...
    private List<ShoppingBasketItemDTO> getItem(ShoppingBasket basket){
        return shoppingBasketItemDao.findAllByShoppingBasketId(basket.getId())
                .stream().map(item -> mapper.map(item, ShoppingBasketItemDTO.class)).collect(Collectors.toList());
    }
